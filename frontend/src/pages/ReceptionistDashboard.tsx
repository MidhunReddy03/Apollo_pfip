...
export default function ReceptionistDashboard() {
  ...
  const todaysRegistrations = patients.filter(p => 
    p.created_at?.startsWith(todayDate)
  ).length;

  const checkedInPatients = encounters.filter(e => 
    e.status === 'checked_in' && e.created_at?.startsWith(todayDate)
  ).length;

  const inQueue = encounters.filter(e => 
    (e.status === 'checked_in' || e.status === 'in_progress') && 
    e.created_at?.startsWith(todayDate)
  ).length;

  // Calculate average wait time
  const avgWaitTime = (() => {
    const checkedInEncounters = encounters.filter(e => 
      e.status === 'in_progress' && e.created_at?.startsWith(todayDate)
    );
    if (checkedInEncounters.length === 0) return '0m';
    
    const totalWaitMs = checkedInEncounters.reduce((sum, e) => {
      if (e.check_in_time) {
        return sum + (new Date(e.updated_at).getTime() - new Date(e.check_in_time).getTime());
      }
      return sum;
    }, 0);
    
    const avgMinutes = Math.round(totalWaitMs / checkedInEncounters.length / 60000);
    return `${avgMinutes}m`;
  })();
  ...
  const recentPatients = patients
    ...
    .slice(0, 5)
    .map(patient => {
      // Find latest encounter for this patient
      ...
      return {
        ...
        type: patient.patient_type || 'OP',
        time: patient.created_at ? new Date(patient.created_at).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }) : 'N/A',
        status: latestEncounter?.status || 'Registered',
        priority: latestEncounter?.priority || 'normal',
      };
    });
  ...
  const quickActions = [
    ...
    { title: 'Today\'s Stats', description: 'View daily summary', icon: Clock, color: 'medical', action: () => navigate('/receptionist/stats') },
  ];
        ...
        <main className="p-8 space-y-6">
          ...
          <div className="card p-0 overflow-hidden animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h2 className="font-display font-semibold text-slate-900">Recent Patients ({recentPatients.length})</h2>
              <button
